...
public class Bird : Unit
{
    ...
    private void OnCollisionStay2D(Collision2D collision)
    {
        ...
        if (collision.gameObject.CompareTag("Box") && Mathf.Abs(hitDir) < 45f && isTouchingGround)
        {
        ...
        if (collision.gameObject.CompareTag("Box") && theBox != null && theBox.isClimbable)
        {
            ...
            if (hitDir >= 45f && hitDir < 147f && stateMachine.currentState.state == State.Move)
            {
                if (isTouchingGround)
                {
            ...
            else if (hitDir <= -45f && hitDir > -147f && stateMachine.currentState.state == State.Move)
            {
                if (isTouchingGround)
                {
                    if (isMovingRight)
